...
Title.Position = UDim2.new(0, 40, 0, 0) -- Adjusted for CloseBtn
Title.Text = "UziHub V1 | OBSIDIAN GUI"
...
SideDesc.Size = UDim2.new(0, 140, 1, -20)
SideDesc.Position = UDim2.new(1, -150, 0, 10)
SideDesc.BackgroundColor3 = Theme.Surface
SideDesc.BorderSizePixel = 0
Instance.new("UICorner", SideDesc)

local DescText = Instance.new("TextLabel", SideDesc)
DescText.Size = UDim2.new(1, -10, 1, -10)
DescText.Position = UDim2.new(0, 5, 0, 5)
DescText.BackgroundTransparency = 1
DescText.TextColor3 = Theme.Text
DescText.TextSize = 12
DescText.Font = Enum.Font.Gotham
DescText.TextXAlignment = Enum.TextXAlignment.Left
DescText.TextYAlignment = Enum.TextYAlignment.Top
DescText.TextWrapped = true
DescText.Text = "Main dev: @puffypillo\nCo dev: @yg7tboy\n\nDiscord:\nhttps://discord.gg/RyxZ5HJmr9"
...
local function applyESP(player)
    ...
    local info = Drawing.new("Text"); info.Visible = false; info.Center = true; info.Outline = true; info.Size = 13; info.Font = 2
    ...
    local function setup(char)
        ...
        highlight.Name = "UziHighlight"; highlight.FillTransparency = 0.5; highlight.OutlineTransparency = 0
        
        local conn
        conn = RunService.RenderStepped:Connect(function()
            if not Toggles["Aimbot + ESP"] or not char.Parent or not isEnemy(player) or hum.Health <= 0 then
                highlight.Enabled = false; tracer.Visible = false; info.Visible = false
                if not char.Parent then highlight:Destroy(); tracer:Remove(); info:Remove(); conn:Disconnect() end
                return
            end
            local color = Color3.fromHSV(tick() % 5 / 5, 1, 1)
            highlight.Enabled = true; highlight.FillColor = color; highlight.OutlineColor = color
            local screenPos, onScreen = Camera:WorldToViewportPoint(root.Position)
            if onScreen then
                tracer.Color = color; tracer.From = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y)
                ...
                info.Text = player.Name .. "\nHP: " .. math.floor(hum.Health) .. " | " .. dist .. " studs"
                info.Position = Vector2.new(screenPos.X, screenPos.Y - 45); info.Color = color; info.Visible = true
            else tracer.Visible = false; info.Visible = false end
        end)
    end
    player.CharacterAdded:Connect(setup)
    if player.Character then task.spawn(setup, player.Character) end
    ESP_Objects[player] = true
end
...
local function CreateToggle(name, callback)
    ...
    Btn.TextSize = 13; Btn.TextXAlignment = Enum.TextXAlignment.Left; Instance.new("UICorner", Btn)
...
CreateToggle("Speed", function(v) if not v and LocalPlayer.Character then LocalPlayer.Character.Humanoid.WalkSpeed = 16 end end)
CreateSlider("WalkSpeed", 16, 250, 16, function(v) Values.WalkSpeed = v end)
CreateToggle("Jump", function(v) if not v and LocalPlayer.Character then LocalPlayer.Character.Humanoid.JumpPower = 50 end end)
CreateSlider("JumpPower", 50, 500, 50, function(v) Values.JumpPower = v end)

RunService.Stepped:Connect(function() if Toggles.Noclip and LocalPlayer.Character then for _, p in pairs(LocalPlayer.Character:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide = false end end end end)
RunService.Heartbeat:Connect(function() if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then if Toggles.Speed then LocalPlayer.Character.Humanoid.WalkSpeed = Values.WalkSpeed end if Toggles.Jump then LocalPlayer.Character.Humanoid.JumpPower = Values.JumpPower end end end)
RunService.RenderStepped:Connect(function() if Toggles["Aimbot + ESP"] and locking then local target = getClosestHead() if target then Camera.CFrame = CFrame.new(Camera.CFrame.Position, target.Position) end end end)

UserInputService.InputBegan:Connect(function(i, g) if g then return end if i.KeyCode == Enum.KeyCode.RightAlt then Main.Visible = not Main.Visible elseif i.UserInputType == Enum.UserInputType.MouseButton2 then locking = true end end)
